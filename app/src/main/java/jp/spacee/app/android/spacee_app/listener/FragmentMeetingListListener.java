...
import jp.spacee.app.android.spacee_app.fragment.FragmentMeetingList;


public  class  FragmentMeetingListListener  implements  FragmentMeetingList.FragmentInteractionListener
{
	...
	private						List<HashMap<String, String>>	meetingList	= null;
	...
	public FragmentMeetingListListener()
	{
	}
	...
	{
		ListView	meetingList = (ListView)	view.findViewById(R.id.meetingRooms);
	...
	private void retrieveListInfo(View view)
	{
		...
		String  result = SpaceeAppMain.httpCommGlueRoutines.retrieveOfficeInfo("space");
		if (result != null)
		{
			try
			{
...
//					{
						...
						schedList	 = new ArrayList<HashMap<String, String>>();
...
//					}
			...
			String[]	urls = new String[meetingList.size()];
			for (i=0; i<meetingList.size(); i++)
			{
				HashMap<String, String>  map = new HashMap<String, String>();
				map = meetingList.get(i);
	...
	private void redrawMeetingList(ListView listView)
	{
		...
		for (i = 0; i < meetingList.size(); i++)
		{
			...
			mapout.put("remarks",	ReceiptTabApplication.AppContext.getString(R.string.frag_meeting_list_max_psn)
					  + mapin.get("capacity") + ReceiptTabApplication.AppContext.getString(R.string.frag_meeting_list_psn_avail)
					  + mapin.get("book_unit") + ReceiptTabApplication.AppContext.getString(R.string.frag_meeting_list_min_unit));
			meetingAreaInfo.add(mapout);
		}
		adapter = new android.widget.SimpleAdapter(ReceiptTabApplication.AppContext,
													meetingAreaInfo,
													R.layout.layout_list_meetingroom,
													new String[]{"Status", 	"Name", 		"Facility", 		"Price", 		"placeExp",	"remarks"},
													new int   []{R.id.status, R.id.placeName, R.id.facilities, R.id.price,	R.id.placeExp, R.id.remarks})
		{
			@Override
...
//		cvs.drawColor(Color.rgb(255, 255, 240));
		...
		cvs.drawText(ReceiptTabApplication.AppContext.getString(R.string.frag_meeting_list_time_now), 20, 50, paint1);
		for (i=0; i<4; i++) {
